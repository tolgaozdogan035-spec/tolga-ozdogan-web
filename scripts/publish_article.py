...
def main():
    ...
    parser.add_argument("--json_file", help="Doğrudan JSON dosyasından veri oku")
    parser.add_argument("--title", help="Köşe yazısının başlığı")
    parser.add_argument("--slug", help="Yazı URL slug'ı")
    parser.add_argument("--date", help="Yayın tarihi")
    parser.add_argument("--read_time", default="5 dk", help="Okuma süresi")
    parser.add_argument("--category", default="Edebiyat & Felsefe", help="Kategori")
    parser.add_argument("--content", help="Yazının tam metni")
    parser.add_argument("--quote", default="", help="Alıntı cümlesi")
    parser.add_argument("--tags", default="#Edebiyat,#Felsefe,#KaosunMimarıİnsan", help="Etiketler")
    ...
    if args.json_file and os.path.exists(args.json_file):
        ...
        read_time = data.get("read_time", "5 dk")
        category = data.get("category", "Edebiyat & Felsefe")
        content = data.get("content", "")
        quote = data.get("quote", "")
        tags_list = data.get("tags", ["#Edebiyat", "#Felsefe"])
    else:
        title = args.title
        slug = args.slug
        date_str = args.date
    ...
    if not title or not slug or not content:
        print("Hata: Başlık, slug veya içerik eksik.")
        sys.exit(1)
    ...
    if os.path.exists(article_filename):
        ...
        content_html = format_content(content, quote)
